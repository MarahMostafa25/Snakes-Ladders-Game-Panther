...
public class boardController implements Initializable {
	...
	private int calLabelValue(int row, int col) // this function calculates the label value with a givin row nd col
	{
		int labelValue;
		if (row % 2 == 0) {
	...
	private final Color[] allowedColors = { Color.web("white"), Color.web("#FDA50F"), };
	...
	public void setSnakes() {
		...
		while ((labelValue == x * x) || (labelValue < 13) || col == x - 1 || col == x - 2) // Can't be at the end or the
																							// start
			labelValue = setObjCheckOcuupied();
		...
		for (int i = 0; i < 2; i++) {
			...
			snake = new Snake(labelValue, labelValue, SnakeColor.green);
			ocuupiedCells.put(labelValue, true);
			snakesOnBoard.put(labelValue, snake);
			if (i == 0)
	...
	public void setLaddrs() {
		...
		int labelValue = 0;
		/**************** ladder in 1 level******************/
			while ((labelValue > 89) || (labelValue < 2)) // Can't be at the end or the start
				labelValue = setObjCheckOcuupied();
			HashMap<Integer, Integer> map = boardCells.get(labelValue);
			int row = 0;
			int col = 0;
			...
			int endValue = calLabelValue(row - 1, col+1);
			ladder = new Ladder(1,labelValue,endValue,0,Level.Medium);
			ocuupiedCells.put(labelValue, true);
			laddersOnBoard.put(labelValue, ladder);
			setSnakeToBoardView(ladder1, 100, 100, row, col);
	...
	private void configureGridPane() {
		// Set constraints to ensure that cells fill the available space without
		// resizing the GridPane
		for (int i = 0; i < x; i++) {
