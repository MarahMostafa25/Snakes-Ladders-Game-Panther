...
//	setSnakeToBoardView(ladder1, 55, 100, 0, 0);
//  setSnakeToBoardView(ladder2, 70, 300, 4, 5);
//  setSnakeToBoardView(ladder3, 130, 340, 0, 5);
// 	setSnakeToBoardView(ladder4, 240,330, 2, 0);

	private ImageView ladder1 = new ImageView(new Image("/Images/ladder1.png"));
	private ImageView ladder2 = new ImageView(new Image("/Images/ladder3.png"));
	private ImageView ladder3= new ImageView(new Image("/Images/ladder_rotate.png"));
	private ImageView ladder4 = new ImageView(new Image("/Images/ladder4Easy.png"));
	...
	public void startGame()
	{
		...
		if(Level2.equals(Level.Easy))
		{
			dice=new Dice(0,8,Level.Easy); // 8 = number of faces 0-4 (5) and 1 option for each one of the questions 
		}
	...
	private boolean display_question(int diceResult,Player p,String type)
	{
		...
		if(diceResult==5)// we got easy question
		{
	...
	private int check_square(int position,Player p,String type)
	{
		...
		if(res!=null)
		{
			 ...
			 if(res.getType()=="Easy")
			 {
				 display_question(5,p,type);
			 }
			 if(res.getType()=="Medium")
			 {
				 display_question(6,p,type);
			 }
			 if(res.getType()=="Hard")
			 {
	...
	public void setSnakes()
	{
		...
		setSnakeToBoardView(red1,50,50,row,col);
		...
		while((labelValue == x*x) || (labelValue < 22)||ocuupiedCells.get(endValue)==null||ocuupiedCells.get(endValue)==true||checkEmptyCells(labelValue,endValue)==0) { // Can't be at the end or the start
			labelValue = setObjCheckOcuupied();
		...
		snakesOnBoard.put(labelValue, snake);
		setSnakeToBoardView(bl,100,300,row,col);
		...
		while((labelValue == x*x) || (labelValue < 10)||col==x-1||col==x-2||ocuupiedCells.get(endValue)==null||ocuupiedCells.get(endValue)==true||checkEmptyCellsForYellow(row,col)==0) { // Can't be at the end or the start
			labelValue = setObjCheckOcuupied();
			endValue = calcEnd(labelValue,1,2);
			map =  boardCells.get(labelValue); 
			for (Map.Entry<Integer, Integer> entry : map.entrySet()) {
		...
		ocuupiedCells.put(endValue, true);
		snakesOnBoard.put(labelValue, snake);
		setSnakeToBoardView(yellow,180,150,row,col);
		/************Green Snake**************************/
		...
		ocuupiedCells.put(endValue, true);
		snakesOnBoard.put(labelValue, snake);
		setSnakeToBoardView(gr1,100,260,row,col);
	...
	public void setLaddrs() {
		...
		int labelValue = setObjCheckOcuupied();
		endValue = calcEnd(labelValue,4,2);
		map = boardCells.get(labelValue);
		row = 0;
		col = 0;
		for (Map.Entry<Integer, Integer> entry : map.entrySet()) {
		...
		while (row>2||col>4||ocuupiedCells.get(endValue)==null||ocuupiedCells.get(endValue)==true||checkEmptyCellsRotate(labelValue,endValue)==0) {
			labelValue = setObjCheckOcuupied();
			endValue = calcEnd(labelValue,4,2);
			map = boardCells.get(labelValue);
			for (Map.Entry<Integer, Integer> entry : map.entrySet()) {
		...
		setSnakeToBoardView(ladder4, 240,330, row, col);
		...
		while ((labelValue <= 22|| col==x-1)||ocuupiedCells.get(endValue)==null||ocuupiedCells.get(endValue)==true||checkEmptyCells(labelValue,endValue)==0) { // Can't be less than 31 
			labelValue = setObjCheckOcuupied();
			endValue = calcEnd(labelValue,3,1);
			map = boardCells.get(labelValue);
			for (Map.Entry<Integer, Integer> entry : map.entrySet()) {
		...
		laddersOnBoard.put(labelValue, ladder);
		setSnakeToBoardView(ladder3, 130, 340, row, col);
		...
		labelValue = setObjCheckOcuupied();
		endValue = calcEnd(labelValue,2,0);
		while ((labelValue <=15)||ocuupiedCells.get(endValue)==true||ocuupiedCells.get(endValue)==null||checkEmptyCells(labelValue,endValue)==0) { // Can't be at the end or the start
			labelValue = setObjCheckOcuupied();
			endValue = calcEnd(labelValue,2,0);
		}
		map = boardCells.get(labelValue);
		...
		setSnakeToBoardView(ladder2, 70, 300, row, col);
		System.out.println("ladder2");
		
		/**************** ladder 1 in esay level******************/
		labelValue = setObjCheckOcuupied();
		endValue = calcEnd(labelValue,1,0);

		while ((labelValue <8)||ocuupiedCells.get(endValue)==null||ocuupiedCells.get(endValue)==true) { // Can't be at the end or the start
			labelValue = setObjCheckOcuupied();
			endValue = calcEnd(labelValue,1,0);
		...
		ladder = new Ladder(labelValue,endValue,1,Level.Easy);
		ocuupiedCells.put(labelValue, true);
		ocuupiedCells.put(endValue, true);
		laddersOnBoard.put(labelValue, ladder);
